...
The tests here are selected for their performance interest — text search, large expansions, multiple filters, diverse code system sources — not for coverage breadth. This is not a conformance test; a server may excel at coverage and correctness across the full FHIR terminology API and still rank low here. For broad conformance testing, see the [FHIR TX Ecosystem](https://github.com/HL7/fhir-tx-ecosystem-ig).
...
- **Coverage gaps.** Not all FHIR operations and code systems are covered. Servers that support operations outside the test suite are not rewarded for that coverage. See the [FHIR TX Ecosystem](https://github.com/HL7/fhir-tx-ecosystem-ig) for broad conformance testing.
