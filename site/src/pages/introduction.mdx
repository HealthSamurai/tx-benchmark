...
| **SS** — Subsumption | `$subsumes` for SNOMED CT hierarchies |
...
**Composite Score** — A single 0–100 number summarising overall performance. The top server in each run scores 100; all others are a percentage of that. See the [Methodology](/methodology) page for the formula.
...
**Preflight dots** — Each coloured dot represents one test case. Green = passed correctness check and was benchmarked. Red = returned an incorrect response (excluded from scoring). Grey = not supported or returned 404/501.
